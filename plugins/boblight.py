...
class Boblight():
  ...
  def bob_addpixel(self,rgb,light=-1):
    if self.boblightLoaded and self.connected:
      self.libboblight.boblight_addpixel(self.bobHandle, light, rgb)
  ...
  def bob_set_static_color(self,rgb,light=-1):
    res = False
    if self.boblightLoaded and self.connected:
      self.bob_addpixel(rgb,light)
